...
// Importação dos Middlewares e Controllers
...
// Grupo principal para todas as rotas de administração
// Aplica o middleware, o prefixo de URL '/admin' e o prefixo de nome 'admin.'
Route::middleware(['web', 'auth', IsAdmin::class])
    ...
    // Customize
    Route::controller(CustomizeController::class)->name('customize.')->group(function () {
        Route::get('customize', 'index')->name('index');
        Route::post('customize', 'update')->name('update')->middleware('demo');
    });

    // Updater
    Route::controller(UpdaterController::class)->name('updater.')->group(function () {
    ...
    // Movie
    Route::controller(MovieController::class)->name('movie.')->group(function () {
    ...
    // Episode
    Route::controller(EpisodeController::class)->name('episode.')->group(function () {
    ...
    // Live broadcast
    Route::controller(BroadcastController::class)->name('broadcast.')->group(function () {
    ...
    // Genre
    Route::controller(GenreController::class)->name('genre.')->group(function () {
    ...
    // People
    Route::controller(PeopleController::class)->name('people.')->group(function () {
    ...
    // Plan
    Route::controller(PlanController::class)->name('plan.')->group(function () {
    ...
    // Tool
    Route::controller(ToolController::class)->name('tool.')->group(function () {
        Route::get('tools', 'index')->name('index');
    });

    // Tmdb
    Route::controller(TmdbController::class)->name('tmdb.')->group(function () {
        ...
        // Importar/sincronizar (client-side dirigirá chamadas a estas)
        ...
        // IMPORTANTE: Removidos endpoints de sincronização com progresso no servidor.
        // A sincronização e progresso agora são totalmente client-side (JavaScript),
        // chamando tmdb-store por TMDB ID, com UI de progresso local.
    });

    // Onesignal
    Route::controller(OnesignalController::class)->name('onesignal.')->group(function () {
    ...
    // RequestPost
    Route::controller(RequestPostController::class)->name('request.')->group(function () {
    ...
    // Collection
    Route::controller(CollectionController::class)->name('collection.')->group(function () {
    ...
    // Community
    Route::controller(CommunityController::class)->name('community.')->group(function () {
    ...
    // Advertisement
    Route::controller(AdvertisementController::class)->name('advertisement.')->group(function () {
